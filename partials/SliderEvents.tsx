...
const SliderEvents = ({ title, list }: TProps) => {
  ...
  return (
    <motion.div variants={fadeIn} className="relative my-20">
      ...
      <Swiper
        ...
        loop
        navigation={{
          prevEl: '.prev-btn-slider-2',
          nextEl: '.next-btn-slider-2'
        }}
      >
        {list.map(item => (
          <SwiperSlide>
      ...
      <div className="absolute -top-3 flex right-0 border border-white/20 p-2 rounded-full">
        <div className={`prev-btn-slider-2 bg-primary rounded-full cursor-pointer size-10 grid place-content-center mr-3 hover:bg-[#363636] duration-300`}><RiArrowLeftSLine /></div>
        <div className="next-btn-slider-2 bg-primary rounded-full cursor-pointer size-10 grid place-content-center hover:bg-[#363636] duration-300"><RiArrowRightSLine /></div>
      </div>
    </motion.div>
  )
}
